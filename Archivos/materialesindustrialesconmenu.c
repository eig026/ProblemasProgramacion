...
void listado_selectivo(char *nombre);
...
/* lee la opcion y ejecuta la accion correspondiente */
    ...
    do{ system("cls");
        ...
        printf("\t3.- Clasificar por nombre de material\n");
        printf("\t4.- Listado selectivo\n");
        ...
        case '2': c_secuencial(nombre);
                  break;
        case '3': clasificacion_md(nombre);
                  break;
...
void particion_tramos(char *archi,char *temp1,char *temp2,long n){
	...
	cont=0;
	conmutador=1;
	leer_archivo_material(f,&ventana);
	while (!feof(f)){
		if (conmutador)
		     grabar_archivo_material(f1,ventana);
		else grabar_archivo_material(f2,ventana);
		cont++;
		if (cont==n){
...
void fusion_tramos(char *archi,char *temp1,char *temp2,long n){
      FILE *f,*f1,*f2;
      tipo_material ventana1,ventana2;
      int cont1,cont2;
      ...
      f1=fopen(temp1,"rb");
      f2=fopen(temp2,"rb");
      leer_archivo_material(f1,&ventana1);
      leer_archivo_material(f2,&ventana2);
      while ((!feof(f1))&&(!feof(f2))){
            if (strcmpi(ventana1.nom,ventana2.nom)<=0){
                grabar_archivo_material(f,ventana1);
	   	    cont1++;
                leer_archivo_material(f1,&ventana1);
                if (cont1==n){
                    cont1=0;
                    grabar_archivo_material(f,ventana2);
                    cont2++;
                    leer_archivo_material(f2,&ventana2);
...
void listado_selectivo(char *nombre){
	...
	if((f=fopen(nombre,"rb"))==NULL){
		...
		getch();
		 ...
		 do{  printf("Densidad minima: ");
	         scanf(" %lf",&den);
	     }while(den<=0);
	     cont=0;
	     printf("\n\nMateriales con densidad > %.2f  y RRP > 1.00\n",den);
	     leer_archivo_material(f,&mat);
	     while(!feof(f)){
	     	if((mat.den>den)&&(mat.rrp>1)){
	     		printf("\t%s\n",mat.nom);
	     		cont++;
			 }
			 leer_archivo_material(f,&mat);
